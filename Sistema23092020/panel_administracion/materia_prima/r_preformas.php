...
if ($unidad_medida=="" or $gramaje=="")
{
    print "<script>alert(\"Complete todos los Campos.\");window.location='preformas.php';</script>";
}
elseif($filas>0)
{
    print "<script>alert(\"La preforma ya existe registrada.\");window.location='preformas.php';</script>";
}
else
{
    ...
    while ($row=mysqli_fetch_array($tipo)) {
        ...
        $sql="insert into preformas (unidad_medida, gramaje, usd, tipo_cambio) values('$unidad_medida','$gramaje','$dolar','$tipodeCambio')";
