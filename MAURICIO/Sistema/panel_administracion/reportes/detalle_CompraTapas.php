...
class PDF extends FPDF
{
    ...
    function Header(){
        ...
        $fecha = date("d/m/Y");
        $fecha1="Id del pedido:".$id."                                                                                                                                      Fecha:  ".$fecha;
        $this->Cell(0,10,$fecha1 ,0,0,'C');
...
$pdf->Cell(50,8,utf8_decode('Proveedor'), 1 );
$pdf->Cell(35,8,utf8_decode('Presentación'), 1 );
$pdf->Cell(65,8,utf8_decode('No.De Tapa'), 1 );
//$pdf->Cell(20,8,utf8_decode('Precio ind.'), 1);
$pdf->Cell(25,8,utf8_decode('millarcaja'), 1);
$pdf->Cell(15,8,utf8_decode('Cajas'), 1 );
$pdf->Cell(45,8,utf8_decode('Importe'), 1);
$pdf->Ln();
$pdf->SetFont('Arial','',12);
//$preforma="SELECT PV.nombre_fiscal, T.tamano, T.presentacion, T.millarcaja, DC.id_detalleCompraTapas, DC.codigo_compraTapas, (DC.cantidad*T.precio) importeTapas,DC.cajas FROM  detalle_compratapas DC, tapas T, compra_tapas CT, proveedores PV  WHERE DC.codigo_compraTapas='$id' AND T.id_tapa=DC.id_tapa AND PV.id_proveedor=CT.id_proveedor and CT.codigo_compraTapas=DC.codigo_compraTapas GROUP BY DC.id_detalleCompraTapas  ";
$tapa="SELECT PV.nombre_fiscal, T.tamano, T.presentacion, T.millarcaja, DC.id_detalleCompraTapas, DC.codigo_compraTapas, (T.millarcaja*DC.cajas) importeTapas,DC.cajas FROM  detalle_compratapas DC, tapas T, compra_tapas CT, proveedores PV  WHERE DC.codigo_compraTapas='$id' AND T.id_tapa=DC.id_tapa AND PV.id_proveedor=CT.id_proveedor and CT.codigo_compraTapas=DC.codigo_compraTapas GROUP BY DC.id_detalleCompraTapas";
...
if (mysqli_num_rows($Tapas)>0)
{
    ...
    {
        $total_compra=round($total_compra+$arreglo['importeTapas'],2);
        $pdf->SetFont('Arial','',10); //Arial, negrita, 12 puntos
        $pdf->Cell(8,8,"",0);
        $pdf->Cell(12,8, $id, 1);
        $pdf->Cell(50,8, $arreglo['nombre_fiscal'], 1);
        $pdf->Cell(35,8, $arreglo['presentacion'], 1);
        $pdf->Cell(65,8, $arreglo['tamano'], 1);
        $pdf->Cell(25,8, $arreglo['millarcaja'], 1);
        $pdf->Cell(15,8, $arreglo['cajas'], 1 );
        $pdf->Cell(45,8,number_format($arreglo['importeTapas'],2), 1);
        $id++;
        $vendedor=$arreglo['nombre_fiscal'];

        $pdf->Ln(8);
    }
    $id=base64_decode($_GET['Cod']);
    $sql="SELECT flete FROM `compra_tapas` WHERE id_compraTapas='$id'";
...
$pdf->Cell(185,8,'',0);
    ...
    $pdf->SetFont('Arial','B',12); 
    $pdf->Cell(45,8, '$'.number_format($total_compra,2),1);
    $pdf->Ln();
     $pdf->Cell(185,8,'',0);
    $pdf->SetFont('Arial','B',12);
    $pdf->Cell(25,8, 'Flete', 1);
    $pdf->SetFont('Arial','B',12); 
    $pdf->Cell(45,8, '$'.number_format($total_Flete,2), 1);
    $pdf->Ln(0);
    $pdf->Cell(185,8,'',0);
    $pdf->Ln();
    $pdf->Cell(185,8,'',0);
    ...
    $pdf->SetFont('Arial','B',12); 
    $pdf->Cell(45,8, '$'.number_format(($total_Flete+$total_compra),2), 1);
    $pdf->Ln(0);
...
$pdf->Cell(19,8,'',0);
    ...
    $pdf->Cell(35,8, 'Proveedor'.$vendedor, 1)
?>
