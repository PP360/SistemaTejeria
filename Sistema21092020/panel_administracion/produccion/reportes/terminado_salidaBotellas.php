...
$pdf->Cell(20,8, 'Cantidad', 1,'R');
$pdf->Cell(20,8, 'Fecha', 1,'R' );
$pdf->Cell(22,8, 'Elaboradas',1, 'R' );
$pdf->Cell(20,8,utf8_decode('Devueltas'), 1 );
$pdf->Cell(15,8, 'merma', 1 );
$pdf->Cell(22,8,"Paquetes",1,0,'R');
$pdf->Cell(22,8,"Sueltas.",1,0,'R');
$pdf->Cell(50,8,utf8_decode('tipo botella'), 1);
$pdf->Ln();
$pdf->SetFont('Arial','',9);
...
if (mysqli_num_rows($bote)>0)
{
    ...
    while($arreglo=mysqli_fetch_array($bote))
    {
         $cantidad_paquetes=0;
        $botellas_sueltas=0;
     if ( $arreglo['tipo_botella']=='Botella de 1000 ml de CSN 26')
     {
         $cantidad_paquetes=(int)($arreglo['elaboradas']/102);
         $botellas_sueltas=(int)($arreglo['elaboradas']%102);
         if ($cantidad_paquetes<1)
         {
             $cantidad_paquetes=0;
         }
     }
        ...
        $pdf->Cell(3,8,"",0);
        $pdf->Cell(20,8, $arreglo['id_entregaPreformas'],1,0);
        $pdf->Cell(50,8, $arreglo['gramaje'], 1);   #Descripcion Entreba
        $pdf->Cell(20,8, $arreglo['cantidad'], 1,0,'R');              #Cantidad
        $pdf->Cell(20,8, $fecha_prod, 1);          #Tipo botella
        $pdf->Cell(22,8, $arreglo['elaboradas'], 1,0,'R');              #Merma
        $pdf->Cell(20,8, $arreglo['devolucion'], 1,0,'R');              #Devolucion
        $pdf->Cell(15,8, $arreglo['merma'], 1,0,'R');              #Elaboradas
        $pdf->Cell(22,8,$cantidad_paquetes,1,0,'R');
        $pdf->Cell(22,8,$botellas_sueltas,1,0,'R');
        $pdf->Cell(50,8, $arreglo['tipo_botella'], 1,0,'R');               #Estatus
        $pdf->Ln(50);
        $numeroEntrega++;
