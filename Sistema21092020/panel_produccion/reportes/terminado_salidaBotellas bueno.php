...
$preforma="SELECT P.gramaje, P.unidad_medida, EP1.id_entregaPreformas, DE.cantidad, PB1.id_entregaPreformas, IB1.cantidad, B1.tipo_botella, PB1.estatus FROM entrega_preformas EP1, entrega_preformas EP2, usuarios U1, usuarios U2, detalle_entregapreformas DE,preformas P, produccion_botella PB1, inventario_preformas IP1, inventario_botellas IB1, botellas B1 WHERE U1.id_usuario=EP1.id_usuarioEntrega AND U2.id_usuario=EP1.id_usuarioRecibe  AND EP1.id_entregaPreformas=DE.id_entregaPreformas AND P.id_preforma=DE.id_preforma AND DE.id_entregaPreformas='$id' GROUP BY  EP1.id_entregaPreformas";
...
if (mysqli_num_rows($prefor)>0)
{
    ...
    while($arreglo=mysqli_fetch_array($prefor))
    {
        ...
        $pdf->Cell(25,8, $arreglo['cantidad'], 1);              #Merma
        $pdf->Cell(25,8, $arreglo['cantidad'], 1);              #Devolucion
        $pdf->Cell(25,8, $arreglo['cantidad'], 1);              #Elaboradas
