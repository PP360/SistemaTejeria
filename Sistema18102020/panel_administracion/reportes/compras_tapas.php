  ...
  if ($desde=="" or $hasta=="")
        {
            print "<script>alert(\"Seleccione un Rango de Fechas.\");window.location='../comprar/verCompraTapas.php';</script>";  
        }
...
class PDF extends FPDF
{
    ...
    function Header(){
      ...
      $this->SetFont('Arial','B',13);
        ...
        $this->Cell(190,25,utf8_decode('C O M P R A   D E  T A P A S  '),0,0,'C');
        $this->Ln(8);
...
$pdf->Ln();
$pdf->Cell(6,8,"",0);
$pdf->Cell(50,8, utf8_decode('Código de compra'), 1 );
$pdf->Cell(60,8, 'Fecha de compra', 1 );
$pdf->Cell(70,8,utf8_decode('Importe'), 1);
$pdf->Ln();
$pdf->SetFont('Arial','',12);
$preforma="SELECT CT.id_compraTapas, CT.codigo_compraTapas,DATE(CT.fecha_compra),SUM(CT.total_compra)FROM compra_tapas CT    WHERE  DATE(CT.fecha_compra)  BETWEEN '$desde' AND '$hasta' GROUP BY CT.codigo_compraTapas ";
...
{
    $total_compra=$total_compra+$arreglo['SUM(CT.total_compra)'];
    $pdf->SetFont('Arial','',10); //Arial, negrita, 12 puntos
    $pdf->Cell(6,8,"",0);
    $pdf->Cell(50,8, $id, 1);
    $pdf->Cell(60,8, $arreglo['DATE(CT.fecha_compra)'], 1);
    $pdf->Cell(70,8, '$'.$arreglo['SUM(CT.total_compra)'], 1);
    $id++;
    $pdf->Ln(8);
}
    $pdf->Cell(56,8,'',0);
...
{
  $total=0; 
    print "<script>alert(\"No hay compra de tapas.\");</script>"; 
}
