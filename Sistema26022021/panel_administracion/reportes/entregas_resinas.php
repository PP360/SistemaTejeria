  ...
  if ($desde=="" or $hasta=="")
        {
            print "<script>alert(\"Seleccione un Rango de Fechas.\");window.location='../bodega/salida_resina.php';</script>";  
        }
...
$preforma="SELECT ER1.id_entregaResina, ER1.fecha_entrega,  (U1.nombre_usuario) entrego, (U2.nombre_usuario) recibe FROM entrega_resina ER1, entrega_resina ER2, usuarios U1, usuarios U2 WHERE U1.id_usuario=ER1.id_usuarioEntrega AND U2.id_usuario=ER1.id_usuarioRecibe   AND DATE(ER1.fecha_entrega) BETWEEN '.$desde' AND '$hasta' GROUP BY  ER1.id_entregaResina  ORDER BY ER1.id_entregaResina ASC";
...
while($arreglo=mysqli_fetch_array($prefor))
{
    ...
    $pdf->Cell(5,8,"",0);
    $pdf->Cell(20,8, $arreglo['id_entregaResina'], 1);
